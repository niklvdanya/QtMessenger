...
void ChatController::setupNetworkCallbacks() {
    m_networkClient->setMessageCallback([this](const Message& message) {
        m_view->displayChatMessage(message);
    });
    
    m_networkClient->setConnectionCallback([this](bool connected) {
        m_view->updateConnectionStatus(connected);
    });
    ...
    m_networkClient->setUserListCallback([this](const std::vector<QString>& userList) {
        m_view->updateUserList(userList);
    });
}
