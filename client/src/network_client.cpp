...
NetworkClient::NetworkClient(QObject* parent) : QObject(parent), m_connected(false) {}
...
void NetworkClient::sendMessage(std::string_view message)
{
    ...
    qDebug() << "Sent message to server (" << bytesWritten
             << " bytes): " << QString::fromStdString(std::string(message));
}
